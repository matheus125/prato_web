...
$app->get('/admin/configuracoes/sistema', function () {
    Funcionarios::verifyLogin();

    if (!pc_can_manage_system_settings()) {
        header('Location: /acesso-negado');
        exit;
    }

    $page = new PageAdmin();
    $page->setTpl('sistema-configuracoes', array(
        'settings' => pc_system_settings_load(),
        'success' => $_SESSION['SystemSettingsSuccess'] ?? '',
        'error' => $_SESSION['SystemSettingsError'] ?? ''
    ));

    unset($_SESSION['SystemSettingsSuccess'], $_SESSION['SystemSettingsError']);
});

$app->post('/admin/configuracoes/sistema', function () {
    Funcionarios::verifyLogin();
    ...
    try {
        pc_system_settings_save(array(
            'limite_senhas_dia' => $_POST['limite_senhas_dia'] ?? null,
            'programa_nome' => $_POST['programa_nome'] ?? ''
        ));

        Funcionarios::audit(
            'SISTEMA_CONFIG_UPDATE',
            'SISTEMA',
            null,
            'Configuracoes do sistema atualizadas: limite=' . (int)($_POST['limite_senhas_dia'] ?? 0) . ', programa=' . trim((string)($_POST['programa_nome'] ?? ''))
        );

        $_SESSION['SystemSettingsSuccess'] = 'Configuracoes salvas com sucesso.';
    } catch (Throwable $e) {
        $_SESSION['SystemSettingsError'] = $e->getMessage();
    }

    header('Location: /admin/configuracoes/sistema');
    exit;
});
...
$app->get('/admin/seguranca/auditoria', function () {
    ...
    $sql = new Sql();
    $dataInicio = isset($_GET['data_inicio']) && $_GET['data_inicio'] !== '' ? trim($_GET['data_inicio']) : date('Y-m-d', strtotime('-7 days'));
    $dataFim = isset($_GET['data_fim']) && $_GET['data_fim'] !== '' ? trim($_GET['data_fim']) : date('Y-m-d');
    ...
    if ($busca !== '') {
        $where[] = '(nome_funcionario LIKE :busca OR cpf_usuario LIKE :busca OR acao LIKE :busca OR detalhes LIKE :busca OR ip LIKE :busca)';
        $params[':busca'] = '%' . $busca . '%';
    }
    ...
    function getPdoRelatorioRemoto()
    {
        ...
        foreach ($required as $campo) {
            if (!array_key_exists($campo, $config) || trim((string)$config[$campo]) === '') {
                throw new Exception(
                    'Banco remoto ativado, mas sem configuração completa. Preencha RELATORIO_REMOTE_DB_HOST, RELATORIO_REMOTE_DB_NAME, RELATORIO_REMOTE_DB_USER e RELATORIO_REMOTE_DB_PASSWORD no .env, ou defina RELATORIO_REMOTE_DB_ENABLED=false.'
                );
            }
        }
...
if (!function_exists('dashboardPeriodoTotalSenhas')) {
    function dashboardPeriodoTotalSenhas(Sql $sql, $inicio, $fim)
    {
        $rows = $sql->select("
            SELECT COUNT(*) AS total
            FROM tb_senhas
            WHERE data_refeicao BETWEEN :inicio AND :fim
        ", array(
            ':inicio' => $inicio,
            ':fim' => $fim
        ));
...
if (!function_exists('dashboardVariacaoPeriodo')) {
    function dashboardVariacaoPeriodo($atual, $anterior)
    {
        $atual = (int)$atual;
        ...
        if ($anterior === 0) {
            return array(
                ...
                'status' => $atual > 0 ? 'AUMENTO' : 'ESTAVEL',
        ...
        $status = 'ESTAVEL';
        if ($percentual > 0) $status = 'AUMENTO';
        if ($percentual < 0) $status = 'QUEDA';
...
if (!function_exists('dashboardComparativosIa')) {
    function dashboardComparativosIa(Sql $sql, $baseDate)
    {
        $base = new DateTimeImmutable($baseDate);
        $inicioSemana = $base->modify('monday this week');
        $fimSemanaAnterior = $inicioSemana->modify('-1 day');
        $inicioSemanaAnterior = $inicioSemana->modify('-7 days');
        $inicioMes = $base->modify('first day of this month');
        $fimMesAnterior = $inicioMes->modify('-1 day');
        $inicioMesAnterior = $inicioMes->modify('-1 month');
        $inicioAno = $base->modify('first day of january this year');
        $fimAnoAnterior = $inicioAno->modify('-1 day');
        $inicioAnoAnterior = $inicioAno->modify('-1 year');

        $periodos = array(
            'dia' => array(
                'label' => 'Hoje x ontem',
                'inicio_atual' => $base->format('Y-m-d'),
                'fim_atual' => $base->format('Y-m-d'),
                'inicio_anterior' => $base->modify('-1 day')->format('Y-m-d'),
                'fim_anterior' => $base->modify('-1 day')->format('Y-m-d')
            ),
            'semana' => array(
                'label' => 'Semana atual x semana anterior',
                'inicio_atual' => $inicioSemana->format('Y-m-d'),
                'fim_atual' => $base->format('Y-m-d'),
                'inicio_anterior' => $inicioSemanaAnterior->format('Y-m-d'),
                'fim_anterior' => $fimSemanaAnterior->format('Y-m-d')
            ),
            'mes' => array(
                'label' => 'Mes atual x mes anterior',
                'inicio_atual' => $inicioMes->format('Y-m-d'),
                'fim_atual' => $base->format('Y-m-d'),
                'inicio_anterior' => $inicioMesAnterior->format('Y-m-d'),
                'fim_anterior' => $fimMesAnterior->format('Y-m-d')
            ),
            'ano' => array(
                'label' => 'Ano atual x ano anterior',
                'inicio_atual' => $inicioAno->format('Y-m-d'),
                'fim_atual' => $base->format('Y-m-d'),
                'inicio_anterior' => $inicioAnoAnterior->format('Y-m-d'),
                'fim_anterior' => $fimAnoAnterior->format('Y-m-d')
            )
        );

        $out = array();
        foreach ($periodos as $key => $periodo) {
            $atual = dashboardPeriodoTotalSenhas($sql, $periodo['inicio_atual'], $periodo['fim_atual']);
            $anterior = dashboardPeriodoTotalSenhas($sql, $periodo['inicio_anterior'], $periodo['fim_anterior']);
            $variacao = dashboardVariacaoPeriodo($atual, $anterior);

            $out[] = array_merge($periodo, array(
                'periodo' => $key,
                'atual' => $atual,
                'anterior' => $anterior,
...
if (!function_exists('dashboardCadastroComparativos')) {
    function dashboardCadastroComparativos(Sql $sql, $baseDate)
    {
        $base = new DateTimeImmutable($baseDate);
        $inicioMes = $base->modify('first day of this month')->format('Y-m-d');
        $fimMes = $base->format('Y-m-d');
        $inicioMesAnterior = $base->modify('first day of previous month')->format('Y-m-d');
        $fimMesAnterior = $base->modify('last day of previous month')->format('Y-m-d');

        $tabelas = array(
            'titulares' => 'tb_titular',
            'dependentes' => 'tb_dependentes'
        );

        $out = array();
        foreach ($tabelas as $key => $table) {
            $atualRows = $sql->select("
                SELECT COUNT(*) AS total
                FROM {$table}
                WHERE DATE(registration_date) BETWEEN :inicio AND :fim
            ", array(':inicio' => $inicioMes, ':fim' => $fimMes));

            $anteriorRows = $sql->select("
                SELECT COUNT(*) AS total
                FROM {$table}
                WHERE DATE(registration_date) BETWEEN :inicio AND :fim
            ", array(':inicio' => $inicioMesAnterior, ':fim' => $fimMesAnterior));

            $atual = isset($atualRows[0]['total']) ? (int)$atualRows[0]['total'] : 0;
            $anterior = isset($anteriorRows[0]['total']) ? (int)$anteriorRows[0]['total'] : 0;
            $variacao = dashboardVariacaoPeriodo($atual, $anterior);
...
if (!function_exists('dashboardAlertasMaioridade')) {
    function dashboardAlertasMaioridade(Sql $sql)
    {
        return $sql->select("
            SELECT
                d.id,
                d.nome,
                d.id_titular,
                t.nome_completo AS titular,
                d.data_nascimento,
                TIMESTAMPDIFF(YEAR, d.data_nascimento, CURDATE()) AS idade_atual,
                DATE_ADD(d.data_nascimento, INTERVAL 18 YEAR) AS data_maioridade,
                DATEDIFF(DATE_ADD(d.data_nascimento, INTERVAL 18 YEAR), CURDATE()) AS dias_para_18
            FROM tb_dependentes d
            LEFT JOIN tb_titular t ON t.id = d.id_titular
            WHERE d.data_nascimento IS NOT NULL
              AND d.data_nascimento <> '0000-00-00'
              AND DATEDIFF(DATE_ADD(d.data_nascimento, INTERVAL 18 YEAR), CURDATE()) BETWEEN 0 AND 30
            ORDER BY dias_para_18 ASC, d.nome ASC
            LIMIT 10
        ");
    }
}

if (!function_exists('dashboardRankingClientes')) {
    function dashboardRankingClientes(Sql $sql, $rankingPeriodo, $mes, $ano, $rankingOrder)
    {
        $wherePeriodo = '';
        $params = array();
        ...
        return $sql->select("
            SELECT
                COALESCE(NULLIF(TRIM(s.cliente), ''), 'Sem nome') AS nome,
                COALESCE(NULLIF(TRIM(s.cpf), ''), '-') AS cpf,
                COUNT(*) AS total_refeicoes,
                COUNT(DISTINCT s.data_refeicao) AS frequencia
            FROM tb_senhas s
            WHERE 1 = 1
            {$wherePeriodo}
            GROUP BY COALESCE(NULLIF(TRIM(s.cliente), ''), 'Sem nome'), COALESCE(NULLIF(TRIM(s.cpf), ''), '-')
            ORDER BY {$orderBy}
            LIMIT 10
        ", $params);
    }
...
if (!function_exists('dashboardInsightsIa')) {
    function dashboardInsightsIa(array $comparativosIa, array $cadastrosComparativo, array $alertasMaioridade, array $rankingClientes)
    {
        $insights = array();

        foreach ($comparativosIa as $item) {
            $status = $item['status'];
            if ($status === 'ESTAVEL') {
                continue;
            }
            ...
            $insights[] = array(
                'titulo' => 'Comparativo de vendas - ' . $item['periodo'],
                'mensagem' => $item['label'] . ': ' . strtolower($status) . ' de ' . abs((int)$item['diferenca']) . ' refeicao(oes), variacao de ' . number_format(abs((float)$item['variacao_percentual']), 1, ',', '.') . '%.'
            );
        }

        foreach ($cadastrosComparativo as $cadastro) {
            if ($cadastro['status'] === 'ESTAVEL') {
                continue;
            }

            $insights[] = array(
                'titulo' => 'Cadastros de ' . $cadastro['tipo'],
                'mensagem' => 'No mes atual houve ' . strtolower($cadastro['status']) . ' de ' . abs((int)$cadastro['diferenca']) . ' cadastro(s) em relacao ao mes anterior.'
            );
        }

        foreach ($alertasMaioridade as $alerta) {
            $dias = (int)$alerta['dias_para_18'];
            $quando = $dias === 0 ? 'hoje' : ($dias === 1 ? 'amanha' : 'em ' . $dias . ' dias');
            $insights[] = array(
                'titulo' => 'Dependente perto dos 18 anos',
                'mensagem' => 'Atencao: o dependente ' . $alerta['nome'] . ' completa 18 anos ' . $quando . '.'
            );
        }

        if (!empty($rankingClientes)) {
            $top = $rankingClientes[0];
            $insights[] = array(
                'titulo' => 'Cliente com mais compras',
                'mensagem' => ($top['nome'] ?? 'Cliente') . ' lidera o ranking do periodo com ' . (int)$top['total_refeicoes'] . ' refeicao(oes).'
            );
        }
...
$app->get('/admin/api/dashboard/geral', function () {
    ...
    try {
        ...
        foreach ($logsBackup as $item) {
            $mensagem = isset($item['mensagem']) ? (string)$item['mensagem'] : (isset($item['msg']) ? (string)$item['msg'] : '');
