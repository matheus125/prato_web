...
$app->post("/admin/clientes/create", function () {
	...
	try {
		...
		$idTitular = (int)($clientes->getid() ?: 0);

		Funcionarios::audit(
			'TITULAR_CREATE',
			'TITULARES',
			$idTitular > 0 ? $idTitular : null,
			'Titular cadastrado: ' . ($_POST['nome_completo'] ?? '') . '. CPF: ' . ($_POST['cpf'] ?? '') . '.'
		);

		Clientes::setSuccess("Titular cadastrado com sucesso. O cadastro ja esta disponivel para vendas e dependentes.");
...
$app->post("/admin/clientes/vulnerabilidade/create", function () {
	...
	try {
		$idTitular = pc_cliente_salvar_vulnerabilidade($_POST);

		Funcionarios::audit(
			'TITULAR_CREATE',
			'TITULARES',
			$idTitular > 0 ? $idTitular : null,
			'Titular em vulnerabilidade cadastrado: ' . ($_POST['nome_completo'] ?? '') . '.'
		);

		Clientes::setSuccess("Titular em vulnerabilidade cadastrado com sucesso.");
		header("Location: /admin/clientes");
		exit;
	} catch (Exception $e) {
		Clientes::setError($e->getMessage());
...
$app->post("/admin/clientes/vulnerabilidade/update", function () {
	...
	try {
		...
		Funcionarios::audit(
			...
			'TITULARES',
			$id,
			'Titular em vulnerabilidade atualizado: ' . ($_POST['nome_completo'] ?? '') . '.'
		);

		Clientes::setSuccess("Titular em vulnerabilidade atualizado com sucesso.");
		header("Location: /admin/clientes");
		exit;
	} catch (Exception $e) {
		Clientes::setError($e->getMessage());
...
$app->post("/admin/clientes/update", function () {
	...
	try {
		...
		Funcionarios::audit(
			...
			'TITULARES',
			$id_titular,
			'Titular atualizado: ' . ($titularAntes[0]['nome_completo'] ?? '') . ' -> ' . ($_POST['nome_completo'] ?? '') . '. CPF: ' . ($_POST['cpf'] ?? '') . '.'
		);

		Clientes::setSuccess("Dados do titular atualizados com sucesso.");
		header("Location: /admin/clientes");
		exit;
	} catch (Exception $e) {
		Clientes::setError($e->getMessage());
...
$app->post("/admin/clientes/:id/delete", function ($id) {
	...
	try {
		...
		Funcionarios::audit(
			...
			'TITULARES',
			$id,
			'Titular excluido: ' . ($titularAntes[0]['nome_completo'] ?? '') . '. CPF: ' . ($titularAntes[0]['cpf'] ?? '') . '.'
		);
