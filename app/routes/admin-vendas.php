...
$app->get("/admin/vendas", function () {
    ...
    Funcionarios::audit('VENDAS_VIEW', 'VENDAS', null, 'Tela de vendas acessada.');

    $page = new PageAdmin();
    $page->setTpl("vendas", array(
        "vendas" => [],
        "limiteSenhasDia" => getLimiteSenhasDia(),
        "programaNome" => pc_system_settings_load()['programa_nome']
    ));
});
...
$app->get("/admin/relatorio/senhas", function () {
    ...
    $page = new PageAdmin();
    $page->setTpl("relatorio-senhas", array(
        "relatorio" => []
    ));
});
...
$app->post("/admin/api/senhas", function () use ($app) {
    ...
    try {
        ...
        $ids = [];
        $numerosSenhas = [];
        $proximoNumeroSenha = $totalAtualBanco + 1;
        ...
        foreach ($itens as $item) {
            ...
            $dedupeKey = function_exists('pc_senha_dedupe_key')
                ? pc_senha_dedupe_key($tipoSenha, $idTitular, $idDependente, $cpf)
                : null;
            ...
            $ids[] = (int)$row[0]["id"];
            $numerosSenhas[] = $proximoNumeroSenha;
            $proximoNumeroSenha++;
        }
        ...
        $sql->query("COMMIT");
        $senhaImpressa = implode(", ", $numerosSenhas);
        Funcionarios::audit(
            'VENDA_SENHAS_CREATE',
            'VENDAS',
            null,
        ...
        echo json_encode([
            ...
            "ids" => $ids,
            "senha" => $senhaImpressa,
            "senhas_numeros" => $numerosSenhas,
            "titular_ja_comprou" => (bool)$titularJaComprou
        ]);
        exit;
    } catch (\Exception $e) {
        ...
        if (strpos($mensagemErro, 'uq_senhas_data_dedupe') !== false || strpos($mensagemErro, '1062') !== false || strpos($mensagemErro, '23000') !== false) {
            $app->response()->status(409);
            $app->response()->header("Content-Type", "application/json; charset=utf-8");
            echo json_encode([
                "ok" => false,
                "error" => "Esta refeicao ja foi liberada hoje para este titular ou dependente. Atualize a tela e confira o historico antes de tentar novamente."
            ]);
            exit;
        }
