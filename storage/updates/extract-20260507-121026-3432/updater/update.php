    ...
    function atualizadorAplicarAtualizacao($force = false)
    {
        ...
        try {
            ...
            if ($checksum !== '') {
                ...
                $hashOk = function_exists('hash_equals')
                    ? hash_equals(strtolower($checksum), strtolower($hash))
                    : strtolower($checksum) === strtolower($hash);

                if (!$hashOk) {
                    throw new Exception('Checksum SHA256 do pacote nao confere.');
                }
            }
