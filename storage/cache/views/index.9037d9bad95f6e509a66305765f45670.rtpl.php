  ...
  <div class="dx-shell">
    <div class="dx-hero">
      ...
      <div class="dx-actions">
        ...
        <button class="dx-btn dx-btn-light" onclick="DX.verificarAtualizacao()">Verificar atualizacoes</button>
      </div>
...
<div class="dx-update-backdrop" id="dxUpdateModal" aria-hidden="true">
  <div class="dx-update-modal" role="dialog" aria-modal="true" aria-labelledby="dxUpdateTitle">
    <div class="dx-update-head">
      <h3 id="dxUpdateTitle">Atualizacao do sistema</h3>
    </div>
    <div class="dx-update-body">
      <div id="dxUpdateInfo">Consultando servidor...</div>
      <div class="dx-update-status" id="dxUpdateStatus">Aguardando acao.</div>
    </div>
    <div class="dx-update-actions">
      <button class="dx-btn dx-btn-light" type="button" onclick="DX.fecharAtualizacao()">Fechar</button>
      <button class="dx-btn dx-btn-primary" type="button" id="dxApplyUpdate" onclick="DX.aplicarAtualizacao()"
        style="display:none">Baixar e instalar</button>
    </div>
  </div>
</div>
...
<script>
  (function () {
    ...
    const els = {
      ...
      updateModal: document.getElementById('dxUpdateModal'),
      updateInfo: document.getElementById('dxUpdateInfo'),
      updateStatus: document.getElementById('dxUpdateStatus'),
      applyUpdate: document.getElementById('dxApplyUpdate')
    };

    let updateInfoAtual = null;
    ...
    function abrirAtualizacao() {
      els.updateModal.classList.add('is-open');
      els.updateModal.setAttribute('aria-hidden', 'false');
    }

    function fecharAtualizacao() {
      els.updateModal.classList.remove('is-open');
      els.updateModal.setAttribute('aria-hidden', 'true');
    }

    function statusAtualizacao(texto, tipo = '') {
      els.updateStatus.className = 'dx-update-status' + (tipo ? ' ' + tipo : '');
      els.updateStatus.textContent = texto;
    }
    ...
    async function verificarAtualizacao() {
      abrirAtualizacao();
      updateInfoAtual = null;
      els.applyUpdate.style.display = 'none';
      els.updateInfo.innerHTML = 'Consultando servidor de atualizacao...';
      statusAtualizacao('Verifique se este computador esta com internet.');

      try {
        await aguardarPintura();
        const resp = await fetch('/admin/api/update/check', { credentials: 'same-origin' });
        const data = await resp.json();
        if (!resp.ok || !data.success) throw new Error(data.message || 'Falha ao verificar atualizacoes.');

        updateInfoAtual = data;
        els.updateInfo.innerHTML = `
          <div><strong>Versao local:</strong> ${esc(data.local_version || '-')}</div>
          <div><strong>Versao online:</strong> ${esc(data.remote_version || '-')}</div>
          <div style="margin-top:10px">${esc(data.notes || 'Sem notas de versao.')}</div>
        `;

        if (data.has_update) {
          els.applyUpdate.style.display = '';
          statusAtualizacao('Nova versao disponivel. O sistema fara backup antes de substituir os arquivos.', 'ok');
        } else {
          statusAtualizacao('O sistema ja esta na versao mais recente.', 'ok');
        }
      } catch (e) {
        els.updateInfo.innerHTML = 'Nao foi possivel verificar a atualizacao.';
        statusAtualizacao((e && e.message ? e.message : 'Falha de conexao.') + ' Tenha internet para sincronizar os dados da atualizacao.', 'err');
      }
    }

    async function aplicarAtualizacao() {
      if (!updateInfoAtual || !updateInfoAtual.has_update) return;
      if (els.applyUpdate.disabled) return;

      els.applyUpdate.disabled = true;
      statusAtualizacao('Baixando ZIP, criando backup e aplicando arquivos. Aguarde...', '');

      try {
        await aguardarPintura();
        const resp = await fetch('/admin/api/update/apply', {
          method: 'POST',
        ...
        const data = await resp.json();
        if (!resp.ok || !data.success) throw new Error(data.message || 'Falha ao aplicar atualizacao.');

        const copied = data.files && data.files.copied ? data.files.copied : 0;
        const skipped = data.files && data.files.skipped ? data.files.skipped : 0;
        els.updateInfo.innerHTML = `
          <div><strong>Versao instalada:</strong> ${esc(data.remote_version || data.local_version || '-')}</div>
          <div><strong>Arquivos atualizados:</strong> ${esc(copied)}</div>
          <div><strong>Arquivos protegidos/ignorados:</strong> ${esc(skipped)}</div>
        `;
        statusAtualizacao(data.message || 'Atualizacao aplicada com sucesso.', 'ok');
        els.applyUpdate.style.display = 'none';
        setTimeout(() => window.location.reload(), 1800);
      } catch (e) {
        statusAtualizacao((e && e.message ? e.message : 'Falha ao aplicar atualizacao.') + ' Corrija a conexao e tente novamente.', 'err');
      } finally {
    ...
    window.DX = { atualizar, rodarBackup, verificarAtualizacao, aplicarAtualizacao, fecharAtualizacao };
    atualizar();
  })();
</script>
