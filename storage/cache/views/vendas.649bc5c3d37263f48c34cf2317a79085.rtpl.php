<?php if(!class_exists('Rain\Tpl')){exit;}?><style>
    ...
    .fechamento-feedback.is-warning {
        border-color: #fde68a;
    ...
    .fechamento-feedback.is-success {
        border-color: #bbf7d0;
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content modal-fechamento-premium">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-outline-primary" id="btnReenviarSyncFechamento"
                                    style="display:none;">Reenviar nuvem</button>
...
<script>
    ...
    const TICKET_CONFIG = {
        ...
        programa: "<?php echo htmlspecialchars( $programaNome, ENT_COMPAT, 'UTF-8', FALSE ); ?>"
    };
    ...
    function criarErroSyncPendente(mensagem) {
        const erro = new Error(mensagem);
        erro.syncPendente = true;
        return erro;
    }

    function mostrarBotaoReenvioSync(exibir) {
        const btnReenviar = document.getElementById("btnReenviarSyncFechamento");
        if (btnReenviar) {
            btnReenviar.style.display = exibir ? "inline-flex" : "none";
        }
    }

    async function sincronizarFechamentoRemoto(dataRef, opcoes = {}) {
        if (navigator.onLine === false) {
            throw criarErroSyncPendente(
                "Fechamento salvo localmente, mas sem internet para sincronizar com a nuvem. Conecte a rede e clique em Reenviar nuvem."
            );
        }

        let resp;
        try {
            resp = await fetch(API.syncRemoto, {
                method: "POST",
                headers: {
                    "Accept": "application/json",
                    "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"
                },
                body: "data=" + encodeURIComponent(dataRef) + "&force=" + (opcoes.force ? "1" : "0")
            });
        } catch (e) {
            throw criarErroSyncPendente(
                "Nao foi possivel acessar a nuvem. Verifique a internet e clique em Reenviar nuvem."
            );
        }
        ...
        if (!resp.ok || !json || json.success !== true) {
            const erro = criarErroSyncPendente(
                json?.message || json?.error || "Falha ao sincronizar com a nuvem. Verifique a internet e tente reenviar."
            );
            erro.sync = json?.sync || null;
            throw erro;
        }
        ...
        if (!json.sync || json.sync.remoto_ok !== true) {
            ...
            throw criarErroSyncPendente(
                ...
                + " Conecte a internet e clique em Reenviar nuvem."
            );
        }
    ...
    async function reenviarSincronizacaoFechamento() {
        const btnReenviar = document.getElementById("btnReenviarSyncFechamento");
        if (btnReenviar) {
            btnReenviar.disabled = true;
            btnReenviar.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Reenviando...';
        }

        try {
            await sincronizarFechamentoRemoto(getHojeKey(), { force: true });
            mostrarBotaoReenvioSync(false);
            mostrarFeedbackFechamento("Sincronizacao com a nuvem concluida com sucesso.", "success");
            ...
            alert("Dados reenviados para a nuvem com sucesso.");
        } catch (e) {
            console.error(e);
            mostrarBotaoReenvioSync(true);
            mostrarFeedbackFechamento(e?.message || "Nao foi possivel reenviar. Verifique a internet e tente novamente.", "warning");
        } finally {
            if (btnReenviar) {
                btnReenviar.disabled = false;
                btnReenviar.textContent = "Reenviar nuvem";
            }
        }
    }
    ...
    // Limite diario carregado das configuracoes do sistema.
    const limiteSenhasDiaConfig = Number("<?php echo htmlspecialchars( $limiteSenhasDia, ENT_COMPAT, 'UTF-8', FALSE ); ?>");
    const LIMITE_SENHAS_DIA = Number.isFinite(limiteSenhasDiaConfig) ? limiteSenhasDiaConfig : 400;
    ...
    function limparFeedbackFechamento() {
        ...
        [
            ...
            el.textContent = "";
            el.classList.remove("is-warning", "is-success");
        });
    ...
    function mostrarFeedbackFechamento(mensagem, tipo = "erro") {
        const { feedback } = getCamposFechamento();
        if (!feedback) return;

        feedback.textContent = mensagem;
        feedback.classList.remove("is-warning", "is-success");
        if (tipo === "warning") feedback.classList.add("is-warning");
        if (tipo === "success") feedback.classList.add("is-success");
    ...
    async function salvarFechamentoLocal() {
        ...
        try {
            ...
            if (e?.syncPendente) {
                mostrarBotaoReenvioSync(true);
                mostrarFeedbackFechamento(e?.message || "Fechamento salvo localmente. Conecte a internet e reenvie para a nuvem.", "warning");

                const status = document.getElementById("statusSenhas");
                if (status) {
                    status.textContent = "Fechamento salvo localmente. Sincronizacao com a nuvem pendente.";
                }
    ...
    /** Gera senha exibida (1, 2, 3...) baseada no total do BANCO + 1 */
    async function gerarSenha() {
        ...
        const senha = String(Number(total) + 1);
    ...
    async function imprimirSenhaGenerica(senha) {
        ...
        let infoHoje = { total: 0, limite: LIMITE_SENHAS_DIA };
        try {
            infoHoje = await getTotalVendidasHojeBanco();
        } catch (e) {
            alert("Não foi possível consultar a contagem no banco. Impressão cancelada.\n" + (e?.message || e));
            return;
        }

        const totalHoje = Number(infoHoje.total || 0);
        const limiteHoje = Number(infoHoje.limite || LIMITE_SENHAS_DIA);

        if ((totalHoje + qtdRefeicoes) > limiteHoje) {
            const restante = Math.max(0, limiteHoje - totalHoje);
            alert(
                `Limite diário atingido.\n` +
                `Você ainda pode liberar ${restante} refeição${restante === 1 ? "" : "es"} hoje.\n` +
                `Total de senhas vendidas hoje: ${totalHoje}/${limiteHoje}.`
            );
        ...
        try {
            const resultadoVenda = await salvarSenhasNoBanco({
                tipoSenha: "GENERICA",
            ...
            senha = resultadoVenda?.senha || senha;
        } catch (e) {
...
.meta { font-size:8px; }
.info { font-size:28px; line-height:1; font-weight:900; margin-top:1px; }
.feed { height:${feed}mm; }
</style>
</head>
...
<div class="feed"></div>
    ...
    async function imprimirSenha() {
        ...
        let senha = document.getElementById("senhaGerada").value;
...
<div class="feed"></div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.getElementById("btnReenviarSyncFechamento")?.addEventListener("click", reenviarSincronizacaoFechamento);

        window.addEventListener("online", function () {
            const btnReenviar = document.getElementById("btnReenviarSyncFechamento");
            if (btnReenviar && btnReenviar.style.display !== "none") {
                mostrarFeedbackFechamento("Conexao detectada. Clique em Reenviar nuvem para sincronizar os dados pendentes.", "warning");
            }
        });
