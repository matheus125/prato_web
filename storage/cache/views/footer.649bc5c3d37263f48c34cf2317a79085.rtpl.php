...
<script>
  ...
  async function fetchJson(url) {
    ...
    try {
      ...
      throw new Error(`Resposta invalida em ${url}: o servidor retornou HTML em vez de JSON.`);
    }

    if (!res.ok || data.success === false || data.erro === true) {
      throw new Error(data.mensagem || `Erro ${res.status} em ${url}`);
    }
